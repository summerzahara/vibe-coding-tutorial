...
import { ParsedDataset } from "@/lib/parse-upload";
...
export default function DashboardPage() {
  ...
  const [uploadedData, setUploadedData] = useState<ParsedDataset | null>(null);

  // Demo data filters
  const [period, setPeriod] = useState<TimePeriod>("all");
  const [payment, setPayment] = useState<PaymentMethod | "all">("all");

  const filtered = useMemo(() => applyFilters(salesData, period, payment), [period, payment]);
  ...
  const handleUpload = (dataset: ParsedDataset | null) => {
    setUploadedData(dataset);
    if (dataset) setTab("upload");
  };
  ...
  return (
    <main className="flex-1 bg-[#fffaf0] py-10 px-5">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-[#9a9a9a] mt-0.5">
              {tab === "demo" ? "Aug 15 – Oct 7, 2025 · Demo data" : (uploadedData?.fileName ?? "Your data")}
            </p>
          </div>

          {/* Tab switcher */}
          <div className="flex rounded-xl overflow-hidden border border-[#e5e5e5] bg-white self-start sm:self-auto">
            <button
              onClick={() => setTab("demo")}
              className={`px-4 py-2 text-sm font-medium transition-colors ${tab === "demo" ? "bg-[#0a0a0a] text-white" : "text-[#6a6a6a] hover:bg-[#f5f0e0]"}`}
            >
              Demo data
            </button>
            <button
              onClick={() => setTab("upload")}
              className={`px-4 py-2 text-sm font-medium transition-colors ${tab === "upload" ? "bg-[#0a0a0a] text-white" : "text-[#6a6a6a] hover:bg-[#f5f0e0]"}`}
            >
              Your data
            </button>
          </div>
        ...
        {tab === "upload" && (
          <>
            <UploadZone dataset={uploadedData} onData={handleUpload} />
            {uploadedData && <DynamicDashboard dataset={uploadedData} />}
          </>
        )}
        ...
        {tab === "demo" && (
          <>
                ...
                {PERIODS.map(({ value, label }) => (
                  <button
                    ...
                    className={`px-3 py-1.5 text-xs font-medium transition-colors ${
                      period === value ? "bg-[#0a0a0a] text-white" : "text-[#6a6a6a] hover:bg-[#f5f0e0]"
                    }`}
                  >
                    {label}
                  </button>
              ...
              <div className="flex flex-wrap rounded-xl overflow-hidden border border-[#e5e5e5] bg-white">
                {PAYMENT_FILTERS.map(({ value, label }) => (
                  <button
                    ...
                    className={`px-3 py-1.5 text-xs font-medium transition-colors ${
                      payment === value ? "bg-[#0a0a0a] text-white" : "text-[#6a6a6a] hover:bg-[#f5f0e0]"
                    }`}
                  >
                    {label}
                  </button>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              ...
              />
              <KpiCard label="Total Orders" value={kpis.totalOrders.toString()} accent="bg-white" />
              <KpiCard label="Avg Order Value" value={`$${kpis.avgOrderValue.toFixed(2)}`} accent="bg-white" />
              <KpiCard
                label="Best Seller"
